...
def run(level, display):
    ...
    player.set_coords(level.spawn, start=True)
    player2.set_coords((level.spawn[0] + 300, level.spawn[1]), start=True)
    ...
    while Run:
        ...
        for o in background_objects:
            if "portal" in o.name or "protector" in o.name:
                ...
                if rad >= 1:
                    pygame.draw.circle(display, (255, 255, 255), coords, rad, 1)
